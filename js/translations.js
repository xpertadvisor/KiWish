const translations = {
    en: {
        nav: {
            benefits: "Benefits",
            how_it_works: "How it Works",
            ingredients: "Ingredients",
            science: "Science",
            success_stories: "Success Stories",
            faq: "FAQ",
            order_now: "Order Now"
        },
        hero: {
            new_zealand_formula: "New Zealand Formula",
            title_part1: "Stubborn Fat Reduction &",
            title_part2: "Clean Burn",
            title_part3: "Formula",
            description: "A refreshing, plant-powered blend designed to gently cleanse your body, support digestion, reduce bloating, and revitalize your glow—without harsh ingredients.",
            start_journey: "Start Your Journey",
            watch_video: "Watch Video",
            trusted_customers: "Trusted by 10,000+ happy customers",
            clinically_proven: "Clinically Proven",
            efsa_approved: "EFSA Approved",
            image_alt: "KiWish Product Box and Kiwis"
        },
        video_section: {
            title: "See KiWish in Action",
            description: "Discover how our natural formula works to cleanse, boost, and revitalize your body from within.",
            image_alt: "Product Video Thumbnail"
        },
        benefits: {
            subtitle: "Why KiWish?",
            title: "Holistic Wellness in Every Sip",
            description: "More than just a detox. It's a comprehensive approach to a healthier, lighter you.",
            detoxification: {
                title: "Detoxification",
                desc: "Gentle gut and liver cleansing that helps remove accumulated toxins effectively."
            },
            metabolism: {
                title: "Metabolism Boost",
                desc: "Enhance your metabolic rate to burn fat naturally, even while you're at rest."
            },
            digestion: {
                title: "Better Digestion",
                desc: "Say goodbye to bloating and enjoy a lighter, more comfortable gut feeling."
            },
            weight_control: {
                title: "Weight Control",
                desc: "Prevents fat storage and converts existing fat into usable energy."
            },
            heart_health: {
                title: "Heart Health",
                desc: "Helps lower cholesterol and maintain healthy blood lipid levels."
            },
            blood_sugar: {
                title: "Blood Sugar",
                desc: "Supports the regulation of blood glucose levels for steady energy."
            },
            gut_flora: {
                title: "Gut Flora",
                desc: "Increases friendly bacteria to support overall digestive wellness."
            },
            absorption: {
                title: "Absorption",
                desc: "Enhanced bioavailability ensures your body gets the nutrients it needs."
            }
        },
        how_it_works: {
            title: "Simple 3-Step Process",
            subtitle: "Transform your body naturally with our scientifically designed flow.",
            step1: {
                title: "Cleanse",
                desc: "Gently detoxes your gut and liver, clearing out toxins & waste to reset your body."
            },
            step2: {
                title: "Boost & Burn",
                desc: "Prevents fat storage & converts existing fat into energy, melting stubborn fat naturally."
            },
            step3: {
                title: "Slim Tummy",
                desc: "Improve digestion & bloating. Your gut feels lighter, and your waistline shows the difference."
            }
        },
        ingredients: {
            subtitle: "Nature's Best",
            title: "Premium Green Ingredients",
            kiwifruit: {
                title: "NZ Kiwifruit",
                desc: "Superior nutrition & consistent quality"
            },
            chlorophyll: {
                title: "Chlorophyll",
                desc: "Natural green pigment from plants"
            },
            green_apple: {
                title: "Green Apple",
                desc: "Rich in antioxidants and fiber"
            },
            cucumber: {
                title: "Cucumber",
                desc: "Hydrating and low-calorie"
            },
            wheat_grass: {
                title: "Wheat Grass",
                desc: "Nutrient-dense superfood"
            },
            patented_title: "3 Patented Powerhouses",
            kionutrime: {
                subtitle: "Multi Enzyme Complex",
                p1: "Binds 800x its weight in fat",
                p2: "EFSA approved health claims",
                p3: "33% more efficient fat-binder"
            },
            sunfiber: {
                subtitle: "Guar Bean Fiber",
                p1: "Lowers cholesterol & glucose",
                p2: "Increases friendly gut bacteria",
                p3: "Relieves constipation naturally"
            },
            digezyme: {
                subtitle: "Plant Chitosan",
                p1: "Prevents weight & fat gain",
                p2: "Enhances nutrient absorption",
                p3: "Supports digestive disorders"
            }
        },
        testimonials: {
            subtitle: "Real Results",
            title: "Success Stories",
            description: "See the transformation and feedback from our happy community."
        },
        faq: {
            daily_ritual: "Daily Ritual",
            step1: "Take 1 sachet daily",
            step2: "Mix with 150ml of water",
            step3: "Enjoy after meals or before sleep",
            tip: "💡 Tip: Mix with cold water or ice for a refreshing taste!",
            image_alt: "Direction for Use",
            common_questions: "Common Questions",
            q1: {
                question: "How long will it take to see results?",
                answer: "Individual results may vary. Most users report feeling lighter and less bloated within the first week. For optimal weight management and metabolic benefits, consistent use for 2-4 weeks is recommended."
            },
            q2: {
                question: "Is KiWish suitable for vegetarians/vegans?",
                answer: "Yes! KiWish is plant-powered and contains non-animal source ingredients, making it suitable for vegetarian and vegan lifestyles."
            },
            q3: {
                question: "Are there any side effects?",
                answer: "KiWish is formulated with natural, hypoallergenic ingredients. However, as with any dietary supplement, some individuals may experience mild digestive changes initially as the body adjusts. This is normal and typically subsides within a few days."
            }
        },
        cta: {
            title: "Ready to Transform Your Health?",
            subtitle: "Join thousands of others who have discovered the natural power of KiWish.",
            button: "Order Now - Start Your Detox",
            shipping: "• Free Shipping on Bundles"
        },
        footer: {
            rights: "© 2026 KiWish. All rights reserved.",
            privacy: "Privacy Policy",
            terms: "Terms of Service",
            contact: "Contact"
        }
    },
    th: {
        nav: {
            benefits: "ประโยชน์",
            how_it_works: "วิธีการทำงาน",
            ingredients: "ส่วนผสม",
            science: "วิทยาศาสตร์",
            success_stories: "เรื่องราวความสำเร็จ",
            faq: "คำถามที่พบบ่อย",
            order_now: "สั่งซื้อเลย"
        },
        hero: {
            new_zealand_formula: "สูตรจากนิวซีแลนด์",
            title_part1: "ลดไขมันดื้อด้าน &",
            title_part2: "สูตรเผาผลาญสะอาด",
            title_part3: "",
            description: "เครื่องดื่มจากพืชที่สดชื่น ออกแบบมาเพื่อทำความสะอาดร่างกายอย่างอ่อนโยน ช่วยย่อยอาหาร ลดอาการท้องอืด และคืนความสดใส—โดยไม่มีส่วนผสมที่รุนแรง",
            start_journey: "เริ่มต้นการเดินทางของคุณ",
            watch_video: "ดูวิดีโอ",
            trusted_customers: "ได้รับความไว้วางใจจากลูกค้ากว่า 10,000+ ราย",
            clinically_proven: "พิสูจน์แล้วทางการแพทย์",
            efsa_approved: "ได้รับการรับรองจาก EFSA",
            image_alt: "กล่องผลิตภัณฑ์ KiWish และกีวี"
        },
        video_section: {
            title: "ดู KiWish ในการทำงานจริง",
            description: "ค้นพบว่าสูตรธรรมชาติของเราทำงานอย่างไรเพื่อทำความสะอาด เสริมสร้าง และฟื้นฟูร่างกายของคุณจากภายใน",
            image_alt: "ภาพตัวอย่างวิดีโอผลิตภัณฑ์"
        },
        benefits: {
            subtitle: "ทำไมต้อง KiWish?",
            title: "สุขภาพองค์รวมในทุกจิบ",
            description: "มากกว่าแค่การดีท็อกซ์ เป็นวิธีการที่ครอบคลุมเพื่อสุขภาพที่ดีขึ้นและตัวเบาขึ้น",
            detoxification: {
                title: "การล้างพิษ",
                desc: "ทำความสะอาดลำไส้และตับอย่างอ่อนโยน ช่วยขจัดสารพิษสะสมอย่างมีประสิทธิภาพ"
            },
            metabolism: {
                title: "เพิ่มการเผาผลาญ",
                desc: "เพิ่มอัตราการเผาผลาญเพื่อเผาผลาญไขมันตามธรรมชาติ แม้ในขณะพักผ่อน"
            },
            digestion: {
                title: "การย่อยอาหารที่ดีขึ้น",
                desc: "บอกลาอาการท้องอืดและเพลิดเพลินกับความรู้สึกสบายท้องและเบาสบาย"
            },
            weight_control: {
                title: "ควบคุมน้ำหนัก",
                desc: "ป้องกันการสะสมไขมันและเปลี่ยนไขมันที่มีอยู่ให้เป็นพลังงานที่ใช้ได้"
            },
            heart_health: {
                title: "สุขภาพหัวใจ",
                desc: "ช่วยลดคอเลสเตอรอลและรักษาระดับไขมันในเลือดให้แข็งแรง"
            },
            blood_sugar: {
                title: "ระดับน้ำตาลในเลือด",
                desc: "สนับสนุนการควบคุมระดับกลูโคสในเลือดเพื่อพลังงานที่สม่ำเสมอ"
            },
            gut_flora: {
                title: "จุลินทรีย์ในลำไส้",
                desc: "เพิ่มแบคทีเรียที่เป็นมิตรเพื่อสนับสนุนสุขภาพทางเดินอาหารโดยรวม"
            },
            absorption: {
                title: "การดูดซึม",
                desc: "เพิ่มชีวปริมาณออกฤทธิ์เพื่อให้ร่างกายได้รับสารอาหารที่ต้องการ"
            }
        },
        how_it_works: {
            title: "กระบวนการ 3 ขั้นตอนง่ายๆ",
            subtitle: "เปลี่ยนร่างกายของคุณตามธรรมชาติด้วยขั้นตอนที่ออกแบบตามหลักวิทยาศาสตร์",
            step1: {
                title: "ทำความสะอาด",
                desc: "ดีท็อกซ์ลำไส้และตับอย่างอ่อนโยน ขจัดสารพิษและของเสียเพื่อรีเซ็ตร่างกาย"
            },
            step2: {
                title: "กระตุ้นและเผาผลาญ",
                desc: "ป้องกันการสะสมไขมันและเปลี่ยนไขมันที่มีอยู่เป็นพลังงาน ละลายไขมันดื้อด้านตามธรรมชาติ"
            },
            step3: {
                title: "หน้าท้องแบนราบ",
                desc: "ปรับปรุงการย่อยอาหารและอาการท้องอืด ลำไส้รู้สึกเบาขึ้น และรอบเอวเริ่มเปลี่ยนแปลง"
            }
        },
        ingredients: {
            subtitle: "ดีที่สุดจากธรรมชาติ",
            title: "ส่วนผสมสีเขียวพรีเมียม",
            kiwifruit: {
                title: "กีวีจากนิวซีแลนด์",
                desc: "โภชนาการที่เหนือกว่าและคุณภาพที่สม่ำเสมอ"
            },
            chlorophyll: {
                title: "คลอโรฟิลล์",
                desc: "เม็ดสีเขียวตามธรรมชาติจากพืช"
            },
            green_apple: {
                title: "แอปเปิ้ลเขียว",
                desc: "อุดมด้วยสารต้านอนุมูลอิสระและไฟเบอร์"
            },
            cucumber: {
                title: "แตงกวา",
                desc: "เติมความชุ่มชื้นและแคลอรี่ต่ำ"
            },
            wheat_grass: {
                title: "ต้นอ่อนข้าวสาลี",
                desc: "ซูเปอร์ฟู้ดที่อุดมด้วยสารอาหาร"
            },
            patented_title: "3 ขุมพลังที่จดสิทธิบัตร",
            kionutrime: {
                subtitle: "คอมเพล็กซ์เอนไซม์หลายชนิด",
                p1: "ดักจับไขมันได้ถึง 800 เท่าของน้ำหนักตัว",
                p2: "คำกล่าวอ้างทางสุขภาพที่ได้รับอนุมัติจาก EFSA",
                p3: "ดักจับไขมันได้ดีขึ้น 33%"
            },
            sunfiber: {
                subtitle: "ไฟเบอร์ถั่วกัวร์",
                p1: "ลดคอเลสเตอรอลและกลูโคส",
                p2: "เพิ่มแบคทีเรียดีในลำไส้",
                p3: "บรรเทาอาการท้องผูกตามธรรมชาติ"
            },
            digezyme: {
                subtitle: "ไคโตซานจากพืช",
                p1: "ป้องกันน้ำหนักและไขมันเพิ่มขึ้น",
                p2: "เพิ่มการดูดซึมสารอาหาร",
                p3: "สนับสนุนความผิดปกติของการย่อยอาหาร"
            }
        },
        testimonials: {
            subtitle: "ผลลัพธ์จริง",
            title: "เรื่องราวความสำเร็จ",
            description: "ดูการเปลี่ยนแปลงและผลตอบรับจากชุมชนที่มีความสุขของเรา"
        },
        faq: {
            daily_ritual: "กิจวัตรประจำวัน",
            step1: "รับประทานวันละ 1 ซอง",
            step2: "ผสมกับน้ำ 150 มล.",
            step3: "ดื่มหลังอาหารหรือก่อนนอน",
            tip: "💡 เคล็ดลับ: ผสมกับน้ำเย็นหรือน้ำแข็งเพื่อรสชาติที่สดชื่น!",
            image_alt: "คำแนะนำการใช้",
            common_questions: "คำถามทั่วไป",
            q1: {
                question: "นานแค่ไหนถึงจะเห็นผล?",
                answer: "ผลลัพธ์อาจแตกต่างกันไปในแต่ละบุคคล ผู้ใช้ส่วนใหญ่รายงานว่ารู้สึกตัวเบาและท้องอืดน้อยลงภายในสัปดาห์แรก เพื่อการควบคุมน้ำหนักและประโยชน์ในการเผาผลาญที่ดีที่สุด แนะนำให้ใช้อย่างต่อเนื่อง 2-4 สัปดาห์"
            },
            q2: {
                question: "KiWish เหมาะสำหรับมังสวิรัติ/วีแกนหรือไม่?",
                answer: "ใช่! KiWish ทำจากพืชและไม่มีส่วนผสมจากสัตว์ จึงเหมาะสำหรับผู้ทานมังสวิรัติและวีแกน"
            },
            q3: {
                question: "มีผลข้างเคียงหรือไม่?",
                answer: "KiWish เป็นสูตรที่มีส่วนผสมจากธรรมชาติและไม่ก่อให้เกิดการแพ้ อย่างไรก็ตาม เช่นเดียวกับผลิตภัณฑ์เสริมอาหารอื่นๆ บางคนอาจมีการเปลี่ยนแปลงในการย่อยอาหารเล็กน้อยในช่วงแรกที่ร่างกายปรับตัว ซึ่งเป็นเรื่องปกติและมักจะหายไปภายในไม่กี่วัน"
            }
        },
        cta: {
            title: "พร้อมที่จะเปลี่ยนสุขภาพของคุณหรือยัง?",
            subtitle: "เข้าร่วมกับผู้คนนับพันที่ค้นพบพลังธรรมชาติของ KiWish",
            button: "สั่งซื้อเลย - เริ่มต้นดีท็อกซ์",
            shipping: "• ส่งฟรีเมื่อสั่งเป็นชุด"
        },
        footer: {
            rights: "© 2026 KiWish. สงวนลิขสิทธิ์",
            privacy: "นโยบายความเป็นส่วนตัว",
            terms: "เงื่อนไขการให้บริการ",
            contact: "ติดต่อ"
        }
    },
    zh: {
        nav: {
            benefits: "功效",
            how_it_works: "原理解析",
            ingredients: "核心成分",
            science: "科学认证",
            success_stories: "成功案例",
            faq: "常见问题",
            order_now: "立即订购"
        },
        hero: {
            new_zealand_formula: "新西兰配方",
            title_part1: "顽固脂肪克星 &",
            title_part2: "清体燃脂",
            title_part3: "配方",
            description: "一款清爽的植物基饮品，旨在温和清洁体内毒素，促进消化，减少腹胀，焕发自然光彩——无任何刺激性成分。",
            start_journey: "开启健康之旅",
            watch_video: "观看视频",
            trusted_customers: "超过 10,000+ 用户信赖",
            clinically_proven: "临床验证",
            efsa_approved: "EFSA 认证",
            image_alt: "KiWish 产品包装和猕猴桃"
        },
        video_section: {
            title: "见证 KiWish 的力量",
            description: "了解我们的天然配方如何由内而外地清洁、提升并焕活您的身体。",
            image_alt: "产品视频缩略图"
        },
        benefits: {
            subtitle: "为什么选择 KiWish?",
            title: "每一口都是全面健康",
            description: "不仅仅是排毒，这是一个让你更健康、更轻盈的全面方案。",
            detoxification: {
                title: "深层排毒",
                desc: "温和清洁肠道和肝脏，有效排出累积毒素。"
            },
            metabolism: {
                title: "代谢提升",
                desc: "提高新陈代谢率，即使在休息时也能自然燃烧脂肪。"
            },
            digestion: {
                title: "改善消化",
                desc: "告别腹胀，享受更轻松、舒适的肠道感觉。"
            },
            weight_control: {
                title: "体重管理",
                desc: "阻止脂肪储存，并将现有脂肪转化为可用能量。"
            },
            heart_health: {
                title: "心脏健康",
                desc: "帮助降低胆固醇，维持健康的血脂水平。"
            },
            blood_sugar: {
                title: "血糖调节",
                desc: "支持血糖水平调节，提供稳定的能量。"
            },
            gut_flora: {
                title: "肠道菌群",
                desc: "增加有益菌群，支持整体消化系统健康。"
            },
            absorption: {
                title: "营养吸收",
                desc: "提高生物利用度，确保身体获得所需营养。"
            }
        },
        how_it_works: {
            title: "简单三步曲",
            subtitle: "通过科学设计的流程，自然地改变您的身体。",
            step1: {
                title: "净化",
                desc: "温和排毒肠道和肝脏，清除毒素和废物，重置身体机能。"
            },
            step2: {
                title: "燃脂",
                desc: "阻止脂肪储存并将现有脂肪转化为能量，自然消融顽固脂肪。"
            },
            step3: {
                title: "塑形",
                desc: "改善消化和腹胀。肠道感觉更轻盈，腰围开始显现变化。"
            }
        },
        ingredients: {
            subtitle: "自然精粹",
            title: "优质绿色成分",
            kiwifruit: {
                title: "新西兰猕猴桃",
                desc: "卓越的营养价值与稳定的品质"
            },
            chlorophyll: {
                title: "叶绿素",
                desc: "源自植物的天然绿色素"
            },
            green_apple: {
                title: "青苹果",
                desc: "富含抗氧化剂和膳食纤维"
            },
            cucumber: {
                title: "黄瓜",
                desc: "补水保湿，低热量"
            },
            wheat_grass: {
                title: "小麦草",
                desc: "营养密集的超级食物"
            },
            patented_title: "三大专利成分",
            kionutrime: {
                subtitle: "多酶复合物",
                p1: "吸附自身重量 800 倍的脂肪",
                p2: "EFSA 批准的健康声明",
                p3: "脂肪吸附效率提高 33%"
            },
            sunfiber: {
                subtitle: "瓜尔豆纤维",
                p1: "降低胆固醇和血糖",
                p2: "增加肠道有益菌",
                p3: "自然缓解便秘"
            },
            digezyme: {
                subtitle: "植物壳聚糖",
                p1: "防止体重和脂肪增加",
                p2: "增强营养吸收",
                p3: "支持消化系统健康"
            }
        },
        testimonials: {
            subtitle: "真实见证",
            title: "成功案例",
            description: "看看我们快乐社区的转变和反馈。"
        },
        faq: {
            daily_ritual: "每日饮用建议",
            step1: "每日 1 袋",
            step2: "加入 150ml 水混合",
            step3: "餐后或睡前饮用",
            tip: "💡 小贴士：加入冷水或冰块，口感更佳！",
            image_alt: "使用说明",
            common_questions: "常见问题",
            q1: {
                question: "多久能看到效果？",
                answer: "效果因人而异。大多数用户在第一周内会感觉身体变轻，腹胀减少。为了获得最佳的体重管理和代谢益处，建议持续使用 2-4 周。"
            },
            q2: {
                question: "KiWish 适合素食者吗？",
                answer: "是的！KiWish 源自植物，不含动物源成分，完全适合素食主义者。"
            },
            q3: {
                question: "有副作用吗？",
                answer: "KiWish 采用天然低敏成分配方。然而，与任何膳食补充剂一样，个别人群在初期可能会经历轻微的消化变化，这是身体适应的正常过程，通常几天内会消失。"
            }
        },
        cta: {
            title: "准备好改变您的健康了吗？",
            subtitle: "加入成千上万已经发现 KiWish 自然力量的人们。",
            button: "立即订购 - 开启排毒之旅",
            shipping: "• 套餐包邮"
        },
        footer: {
            rights: "© 2026 KiWish. 版权所有。",
            privacy: "隐私政策",
            terms: "服务条款",
            contact: "联系我们"
        }
    },
    ms: {
        nav: {
            benefits: "Manfaat",
            how_it_works: "Cara Berfungsi",
            ingredients: "Bahan-bahan",
            science: "Sains",
            success_stories: "Kisah Kejayaan",
            faq: "Soalan Lazim",
            order_now: "Pesan Sekarang"
        },
        hero: {
            new_zealand_formula: "Formula New Zealand",
            title_part1: "Pengurangan Lemak Degil &",
            title_part2: "Pembakaran Bersih",
            title_part3: "Formula",
            description: "Campuran berasaskan tumbuhan yang menyegarkan direka untuk membersihkan badan anda secara lembut, menyokong pencernaan, mengurangkan kembung perut, dan menyerlahkan keserian anda—tanpa bahan kasar.",
            start_journey: "Mulakan Perjalanan Anda",
            watch_video: "Tonton Video",
            trusted_customers: "Dipercayai oleh 10,000+ pelanggan gembira",
            clinically_proven: "Terbukti Secara Klinikal",
            efsa_approved: "Diluluskan EFSA",
            image_alt: "Kotak Produk KiWish dan Buah Kiwi"
        },
        video_section: {
            title: "Lihat KiWish Beraksi",
            description: "Ketahui bagaimana formula semula jadi kami berfungsi untuk membersihkan, meningkatkan, dan memulihkan badan anda dari dalam.",
            image_alt: "Lakaran Video Produk"
        },
        benefits: {
            subtitle: "Kenapa KiWish?",
            title: "Kesejahteraan Holistik dalam Setiap Tegukan",
            description: "Lebih daripada sekadar detoks. Ia adalah pendekatan menyeluruh untuk anda yang lebih sihat dan ringan.",
            detoxification: {
                title: "Detoksifikasi",
                desc: "Pembersihan usus dan hati yang lembut membantu menyingkirkan toksin terkumpul dengan berkesan."
            },
            metabolism: {
                title: "Peningkatan Metabolisme",
                desc: "Tingkatkan kadar metabolisme anda untuk membakar lemak secara semula jadi, walaupun semasa berehat."
            },
            digestion: {
                title: "Pencernaan Lebih Baik",
                desc: "Katakan selamat tinggal kepada kembung perut dan nikmati rasa perut yang lebih ringan dan selesa."
            },
            weight_control: {
                title: "Kawalan Berat Badan",
                desc: "Mencegah penyimpanan lemak dan menukar lemak sedia ada menjadi tenaga yang boleh digunakan."
            },
            heart_health: {
                title: "Kesihatan Jantung",
                desc: "Membantu menurunkan kolesterol dan mengekalkan tahap lipid darah yang sihat."
            },
            blood_sugar: {
                title: "Gula Darah",
                desc: "Menyokong pengawalan tahap glukosa darah untuk tenaga yang stabil."
            },
            gut_flora: {
                title: "Flora Usus",
                desc: "Meningkatkan bakteria baik untuk menyokong kesihatan pencernaan secara keseluruhan."
            },
            absorption: {
                title: "Penyerapan",
                desc: "Ketersediaan bio yang dipertingkatkan memastikan badan anda mendapat nutrien yang diperlukan."
            }
        },
        how_it_works: {
            title: "Proses 3 Langkah Mudah",
            subtitle: "Ubah badan anda secara semula jadi dengan aliran yang direka secara saintifik.",
            step1: {
                title: "Bersihkan",
                desc: "Menyahtoksik usus dan hati anda secara lembut, membersihkan toksin & sisa untuk menetapkan semula badan anda."
            },
            step2: {
                title: "Tingkat & Bakar",
                desc: "Mencegah penyimpanan lemak & menukar lemak sedia ada menjadi tenaga, mencairkan lemak degil secara semula jadi."
            },
            step3: {
                title: "Perut Kempis",
                desc: "Memperbaiki pencernaan & kembung perut. Usus anda terasa lebih ringan, dan pinggang anda menunjukkan perbezaan."
            }
        },
        ingredients: {
            subtitle: "Terbaik dari Alam",
            title: "Bahan Hijau Premium",
            kiwifruit: {
                title: "Buah Kiwi NZ",
                desc: "Nutrisi unggul & kualiti konsisten"
            },
            chlorophyll: {
                title: "Klorofil",
                desc: "Pigmen hijau semula jadi daripada tumbuhan"
            },
            green_apple: {
                title: "Epal Hijau",
                desc: "Kaya dengan antioksidan dan serat"
            },
            cucumber: {
                title: "Timun",
                desc: "Menghidratkan dan rendah kalori"
            },
            wheat_grass: {
                title: "Rumput Gandum",
                desc: "Makanan super padat nutrien"
            },
            patented_title: "3 Kuasa Dipatenkan",
            kionutrime: {
                subtitle: "Kompleks Pelbagai Enzim",
                p1: "Mengikat 800x beratnya dalam lemak",
                p2: "Tuntutan kesihatan yang diluluskan EFSA",
                p3: "Pengikat lemak 33% lebih cekap"
            },
            sunfiber: {
                subtitle: "Serat Kacang Guar",
                p1: "Menurunkan kolesterol & glukosa",
                p2: "Meningkatkan bakteria usus baik",
                p3: "Melegakan sembelit secara semula jadi"
            },
            digezyme: {
                subtitle: "Kitosan Tumbuhan",
                p1: "Mencegah kenaikan berat badan & lemak",
                p2: "Meningkatkan penyerapan nutrien",
                p3: "Menyokong gangguan pencernaan"
            }
        },
        testimonials: {
            subtitle: "Keputusan Nyata",
            title: "Kisah Kejayaan",
            description: "Lihat transformasi dan maklum balas daripada komuniti gembira kami."
        },
        faq: {
            daily_ritual: "Ritual Harian",
            step1: "Ambil 1 sachet setiap hari",
            step2: "Campurkan dengan 150ml air",
            step3: "Nikmati selepas makan atau sebelum tidur",
            tip: "💡 Tip: Campurkan dengan air sejuk atau ais untuk rasa yang menyegarkan!",
            image_alt: "Arahan Penggunaan",
            common_questions: "Soalan Lazim",
            q1: {
                question: "Berapa lama untuk melihat hasil?",
                answer: "Hasil individu mungkin berbeza. Kebanyakan pengguna melaporkan rasa lebih ringan dan kurang kembung dalam minggu pertama. Untuk pengurusan berat badan dan faedah metabolik yang optimum, penggunaan konsisten selama 2-4 minggu disyorkan."
            },
            q2: {
                question: "Adakah KiWish sesuai untuk vegetarian/vegan?",
                answer: "Ya! KiWish berasaskan tumbuhan dan tidak mengandungi bahan sumber haiwan, menjadikannya sesuai untuk gaya hidup vegetarian dan vegan."
            },
            q3: {
                question: "Adakah terdapat kesan sampingan?",
                answer: "KiWish dirumus dengan bahan semula jadi dan hipoalergenik. Walau bagaimanapun, seperti mana-mana suplemen diet, sesetengah individu mungkin mengalami perubahan pencernaan ringan pada mulanya semasa badan menyesuaikan diri. Ini adalah normal dan biasanya reda dalam beberapa hari."
            }
        },
        cta: {
            title: "Bersedia untuk Mengubah Kesihatan Anda?",
            subtitle: "Sertai beribu-ribu orang lain yang telah menemui kuasa semula jadi KiWish.",
            button: "Pesan Sekarang - Mulakan Detoks Anda",
            shipping: "• Penghantaran Percuma untuk Pakej"
        },
        footer: {
            rights: "© 2026 KiWish. Hak cipta terpelihara.",
            privacy: "Dasar Privasi",
            terms: "Terma Perkhidmatan",
            contact: "Hubungi"
        }
    }
};

window.translations = translations;
